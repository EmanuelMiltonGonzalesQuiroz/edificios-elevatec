...
const ViewedPosts = () => {
    ...
    const fetchViewedPosts = async () => {
      ...
      try {
        const q = query(
          ...
          where('views', 'array-contains', currentUser.uid || currentUser.id)
        );
        const querySnapshot = await getDocs(q);
        const postsData = querySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
        setViewedPosts(postsData);
      } catch (error) {
        console.error('Error al obtener publicaciones vistas:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <p className="text-center text-gray-500">Cargando publicaciones vistas...</p>;

  return (
    <div className="p-4 space-y-4">
      {viewedPosts.length === 0 ? (
        <p className="text-center text-gray-500">No has visto ninguna publicación.</p>
      ) : (
