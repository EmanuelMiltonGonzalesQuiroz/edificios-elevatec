...
import { MdOutlineAdminPanelSettings, MdOutlineInventory, MdPhotoLibrary } from 'react-icons/md';
    ...
    <div className={`bg-gray-900 text-white flex flex-col transition-all duration-300 h-full ${isMinimized ? 'w-16' : 'min-w-[30vh]'}`}>
      ...
      <nav className="flex-grow overflow-y-auto">
        <ul>
          ...
          {renderItem('Imágenes', <MdPhotoLibrary />, 'Imágenes', 'images')}
          {renderItem('Pre Venta', <FaUser />, 'PreVenta', 'PreSale')}
          {renderItem('Venta', <FaUsers />, 'Venta', 'Sale')}
          {renderItem('Anticrético', <CgProfile />, 'Anticrético', 'Mortgage')}
          {renderItem('Alquiler', <MdOutlineAdminPanelSettings />, 'Alquiler', 'Rent')}
        </ul>
      </nav>
    </div>
  );
